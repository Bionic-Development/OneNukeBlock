...
public class TntNukeExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(DeathMoney.MOD_ID, "tntnuke");
    ...
    public TntNukeExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public TntNukeExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType, List<BlockPos> affectedBlocks) {
        Explosion explosion = new Explosion(world, entity, damageSource, behavior, x, y, z, power, createFire, destructionType);
        explosion.collectBlocksAndDamageEntities();
        explosion.affectedBlocks.addAll(affectedBlocks);
        explosion.affectWorld(false);
        if (destructionType == net.minecraft.world.explosion.Explosion.DestructionType.NONE) {
            explosion.clearAffectedBlocks();
        }
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            if (!(serverPlayerEntity.squaredDistanceTo(x, y, z) < 12096.0)) continue;

            PacketByteBuf packetByteBuf = PacketByteBufs.create();
            new TntNukeExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
            ServerPlayNetworking.send(serverPlayerEntity, TntNukeExplosionHandler.IDENTIFIER, packetByteBuf);
        }
    }
    ...
    public static class Explosion extends net.minecraft.world.explosion.Explosion {
                ...
                for (k = 0; k < 16; ++k) {
                    ...
                    for (l = 0; l < 16; ++l) {
                        ...
                        for (float h = this.power * (0.9f + this.world.random.nextFloat() * 0.9f); h > 0.0f; h -= 0.22500001f) {
                            ...
                            if (h > 0.0f && (this.behavior.canDestroyBlock(this, this.world, blockPos, blockState, h) || !fluidState.isEmpty())) {
                                set.add(blockPos);
                            }
                            m += d * (double) 0.3f;
                            n += e * (double) 0.3f;
        ...
        public void affectWorld(boolean particles) {
            ...
            if (particles) {
                getAffectedBlocks().forEach(blockPos -> {
                    if (world.getRandom().nextDouble() <= 0.25) {
                        if (this.power < 2.0f || !bl) {
                            this.world.addParticle(ParticleTypes.EXPLOSION, true, blockPos.getX() + 0.5, blockPos.getY() + 0.5, blockPos.getZ() + 0.5, 1.0, 0.0, 0.0);
                        } else {
                            this.world.addParticle(ParticleTypes.EXPLOSION_EMITTER, true, blockPos.getX() + 0.5, blockPos.getY() + 0.5, blockPos.getZ() + 0.5, 1.0, 0.0, 0.0);
                        }
                    }
                });
            }
            if (bl && !particles) {
                ...
                for (BlockPos blockPos : this.affectedBlocks) {
                    ...
                    if (world.getRandom().nextDouble() <= 0.2) {
                        ...
                        customBlockEntity.setPos(blockPos.getX() + 0.5, blockPos.getY() + 1, blockPos.getZ() + 0.5);
                        ...
                        customBlockEntity.setVelocity(vec3d.getX() * world.getRandom().nextDouble(),
                                world.getRandom().nextDouble(), vec3d.getZ() * world.getRandom().nextDouble());
