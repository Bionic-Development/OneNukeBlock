...
public class FireElementalWardenBrain {

    public static final int EMERGE_DURATION = MathHelper.ceil(133.59999f);

    private static final List<SensorType<? extends Sensor<? super FireElementalWardenEntity>>> SENSORS = List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR);
    ...
    public static void updateActivities(FireElementalWardenEntity warden) {
    ...
    public static Brain<?> create(FireElementalWardenEntity warden) {
        Brain.Profile profile = Brain.createProfile(MEMORY_MODULES, SENSORS);
        NbtOps nbtOps = NbtOps.INSTANCE;
        Brain<FireElementalWardenEntity> brain = profile.deserialize(new Dynamic<>(nbtOps, nbtOps.createMap(ImmutableMap.of(nbtOps.createString("memories"), nbtOps.emptyMap()))));
        FireElementalWardenBrain.addCoreActivities(brain);
        FireElementalWardenBrain.addEmergeActivities(brain);
        FireElementalWardenBrain.addDigActivities(brain);
        FireElementalWardenBrain.addIdleActivities(brain);
        FireElementalWardenBrain.addRoarActivities(brain);
        FireElementalWardenBrain.addFightActivities(warden, brain);
        FireElementalWardenBrain.addInvestigateActivities(brain);
        FireElementalWardenBrain.addSniffActivities(brain);
    ...
    private static void addCoreActivities(Brain<FireElementalWardenEntity> brain) {
        brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new StayAboveWaterTask(0.8f),
                ...
                FindRoarTargetTask.create(FireElementalWardenBrain::getPreferredTarget)));
    }

    private static void addEmergeActivities(Brain<FireElementalWardenEntity> brain) {
    ...
    private static void addDigActivities(Brain<FireElementalWardenEntity> brain) {

    }

    private static void addIdleActivities(Brain<FireElementalWardenEntity> brain) {
    ...
    private static void addInvestigateActivities(Brain<FireElementalWardenEntity> brain) {
        brain.setTaskList(Activity.INVESTIGATE, 5, ImmutableList.of(FindRoarTargetTask.create(FireElementalWardenBrain::getPreferredTarget),
                WalkTowardsPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7f)), MemoryModuleType.DISTURBANCE_LOCATION);
    }

    private static void addSniffActivities(Brain<FireElementalWardenEntity> brain) {
        brain.setTaskList(Activity.SNIFF, 5, ImmutableList.of());
    }

    private static void addRoarActivities(Brain<FireElementalWardenEntity> brain) {
        brain.setTaskList(Activity.ROAR, 10, ImmutableList.of(new RoarTask()), MemoryModuleType.ROAR_TARGET);
    }

    private static void addFightActivities(FireElementalWardenEntity warden, Brain<FireElementalWardenEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(
                ForgetAttackTargetTask.create(entity -> !warden.isValidTarget(entity),
                        FireElementalWardenBrain::removeDeadSuspect, false),
                LookAtMobTask.create((LivingEntity entity) -> FireElementalWardenBrain.isTargeting(warden, entity),
                        (float) warden.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE)),
                RangedApproachTask.create(1.2f),
                new FireElementalWardenSonicBoomTask(),
                MeleeAttackTask.create(18)), MemoryModuleType.ATTACK_TARGET);

    }

    private static boolean isTargeting(FireElementalWardenEntity warden, LivingEntity entity2) {
        return warden.getBrain().getOptionalMemory(MemoryModuleType.ATTACK_TARGET).filter(entity -> entity == entity2).isPresent();
    }

    private static void removeDeadSuspect(FireElementalWardenEntity warden, LivingEntity suspect) {
        if (!warden.isValidTarget(suspect)) {
            warden.removeSuspect(suspect);
        }
        FireElementalWardenBrain.resetDigCooldown(warden);
    }
    ...
    public static void lookAtDisturbance(FireElementalWardenEntity warden, BlockPos pos) {
