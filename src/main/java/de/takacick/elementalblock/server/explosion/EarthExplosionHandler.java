...
public class EarthExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(OneElementalBlock.MOD_ID, "earth_explosion");
    ...
    public EarthExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public EarthExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            ...
            new EarthExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
    ...
    public static class Explosion extends net.minecraft.world.explosion.Explosion {
        ...
        public void affectWorld(boolean particles) {
            ...
            if (this.world.isClient) {
                ...
                this.world.playSound(this.x, this.y, this.z, SoundEvents.BLOCK_ROOTED_DIRT_BREAK, SoundCategory.BLOCKS, 4.0f, (1.0f + (this.world.random.nextFloat() - this.world.random.nextFloat()) * 0.2f) * 0.7f, false);
            }
            if (particles) {
                ...
                colors = Arrays.asList(0x564429, 0x886142, 0x886142);

                for (int i = 0; i < this.power * 30; i++) {
                    Vector3f vector3f = Vec3d.unpackRgb(colors.get(random.nextInt(colors.size()))).toVector3f();
                    ParticleEffect particleEffect = new GoopDropParticleEffect(vector3f, (random.nextFloat() * 0.8f) + 0.1f);

                    double d = random.nextGaussian();
                    double e = random.nextGaussian();
                    double f = random.nextGaussian();

                    this.world.addImportantParticle(particleEffect, true, this.x + d, this.y + e, this.z + f, d * 0.5, e * 0.5, f * 0.5);
                }
            }
