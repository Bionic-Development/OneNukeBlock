...
public class EmeraldShopPortalEntity extends Entity {
    ...
    private static final TrackedData<Boolean> DEAD = BionicDataTracker.registerData(new Identifier(EmeraldMoney.MOD_ID, "shop_dead"), TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Integer> PROGRESS = BionicDataTracker.registerData(new Identifier(EmeraldMoney.MOD_ID, "progress"), TrackedDataHandlerRegistry.INTEGER);
    ...
    public void pushAway(Entity entity) {
        ...
        if (equals(emeraldShopPortalEntity)) {
            if (entityProperties.teleportBackFromEmeraldShop()) {
                BionicUtils.sendEntityStatus((ServerWorld) entity.getWorld(), entity, EmeraldMoney.IDENTIFIER, 9);
        ...
        if (!getDataTracker().get(DEAD)) {
            if (getWorld().isClient && getProgress(0) < 1f) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    double f = getWorld().getRandom().nextGaussian() * 0.08;
                    this.getWorld().addParticle(new ColoredParticleEffect(ParticleRegistry.TOTEM_DUST, EmeraldMoneyClient.getEmeraldColor(getWorld().random)),
    ...
    public void setDead(boolean dead) {
        if (getDataTracker().get(DEAD) != dead) {
            BionicUtils.sendEntityStatus((ServerWorld) getWorld(), this, EmeraldMoney.IDENTIFIER, 8);
        }
