package de.takacick.everythinghearts.network;
...
public class HeartmondExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(EverythingHearts.MOD_ID, "heartmond_explosion");
    ...
    public HeartmondExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public HeartmondExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            ...
            new HeartmondExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
            ServerPlayNetworking.send(serverPlayerEntity, HeartmondExplosionHandler.IDENTIFIER, packetByteBuf);
        }
    }
    ...
    public static class Explosion extends net.minecraft.world.explosion.Explosion {
        ...
        public void collectBlocksAndDamageEntities() {
            ...
            for (int v = 0; v < list.size(); ++v) {
                ...
                double ac = (1.0 - w) * ab;
                entity.damage(this.getDamageSource(), 8);
                double ad = ac;
        ...
        public void affectWorld(boolean particles) {
            ...
            if (particles) {
                if (this.power < 2.0f || !bl) {
                    this.world.addParticle(ParticleRegistry.HEART_EXPLOSION, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                } else {
                    this.world.addParticle(ParticleRegistry.HEART_EXPLOSION_EMITTER, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                }
            }
            if (bl && !particles) {
                Util.shuffle(this.affectedBlocks, this.world.random);

                int heartLevel = getCausingEntity() instanceof PlayerProperties playerProperties ? playerProperties.getHeartTouchLevel() : 1;
