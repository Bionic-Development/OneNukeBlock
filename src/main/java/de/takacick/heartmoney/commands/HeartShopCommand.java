package de.takacick.heartmoney.commands;
...
public class HeartShopCommand {

    public static HeartShopPortalEntity HEART_SHOP_PORTAL;

    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        LiteralCommandNode<ServerCommandSource> rootNode = CommandManager.literal("heartshop").requires((source) -> {
                    return source.hasPermissionLevel(2);
                        ...
                        .executes(context -> {
                            ...
                            ((EntityProperties) playerEntity).setHeartShopPortalCooldown(10);

                            if (HEART_SHOP_PORTAL != null) {
                                HEART_SHOP_PORTAL.discard();
                            }

                            HeartShopPortalEntity heartShopPortalEntity = new HeartShopPortalEntity(EntityRegistry.HEART_SHOP_PORTAL, playerEntity.getWorld());
                            heartShopPortalEntity.setPos(playerEntity.getX(), playerEntity.getY(), playerEntity.getZ());
                            heartShopPortalEntity.setYaw(-playerEntity.getYaw());
                            playerEntity.getWorld().spawnEntity(heartShopPortalEntity);

                            HEART_SHOP_PORTAL = heartShopPortalEntity;
                            context.getSource().sendFeedback(Text.of("Set Heart Shop to your current location!"), false);
