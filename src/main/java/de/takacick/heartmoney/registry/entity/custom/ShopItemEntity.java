...
public class ShopItemEntity extends Entity {
    ...
    private static final TrackedData<ItemStack> ITEM_STACK = BionicDataTracker.registerData(new Identifier(HeartMoney.MOD_ID, "shop_item_stack"), TrackedDataHandlerRegistry.ITEM_STACK);
    private static final TrackedData<Integer> PRICE = BionicDataTracker.registerData(new Identifier(HeartMoney.MOD_ID, "shop_item_price"), TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Float> OFFSET = BionicDataTracker.registerData(new Identifier(HeartMoney.MOD_ID, "shop_item_offset"), TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> SCALE = BionicDataTracker.registerData(new Identifier(HeartMoney.MOD_ID, "shop_item_scale"), TrackedDataHandlerRegistry.FLOAT);
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        if (player.getMaxHealth() - ((PlayerProperties) player).getRemoveHearts() <= (int) ((double) getPrice() * 2d)) {
            return ActionResult.CONSUME;
        }

        world.playSound(getX(), getY() + calculateBoundingBox().getYLength() / 2, getZ(), SoundEvents.ENTITY_PLAYER_LEVELUP, SoundCategory.NEUTRAL, 0.4f, 1, true);
        world.playSound(getX(), getY() + calculateBoundingBox().getYLength() / 2, getZ(), SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.NEUTRAL, 0.4f, 1, true);
        if (!world.isClient) {
            ((PlayerProperties) player).addRemoveHearts((int) ((double) getPrice() * 2d));
