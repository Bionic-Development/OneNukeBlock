package de.takacick.illegalwars.client.screen;
...
public class CommandBlockPressurePlateScreen
        extends Screen {
    ...
    private final CommandBlockPressurePlateBlockEntity blockEntity;

    public CommandBlockPressurePlateScreen(CommandBlockPressurePlateBlockEntity blockEntity) {
    ...
    public int getTrackOutputButtonHeight() {
        return 135;
    }
    ...
    protected void init() {
        ...
        this.consoleCommandTextField = new TextFieldWidget(this.textRenderer, this.width / 2 - 150, 50, 300, 20, (Text) Text.translatable("advMode.command")) {
            ...
            protected MutableText getNarrationMessage() {
                return super.getNarrationMessage().append(CommandBlockPressurePlateScreen.this.commandSuggestor.getNarration());
            }
        };
    ...
    protected void syncSettingsToServer(CommandBlockExecutor commandExecutor) {
        this.client.getNetworkHandler().sendPacket(new UpdateCommandBlockC2SPacket(BlockPos.ofFloored(commandExecutor.getPos()),
                this.consoleCommandTextField.getText(), CommandBlockBlockEntity.Type.REDSTONE, commandExecutor.isTrackingOutput(), false, false));
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!this.previousOutputTextField.getText().isEmpty()) {
            context.drawTextWithShadow(this.textRenderer, PREVIOUS_OUTPUT_TEXT, this.width / 2 - 150 + 1, (i += 5 * this.textRenderer.fontHeight + 1 + this.getTrackOutputButtonHeight() - 135) + 4, 0xA0A0A0);
