...
public class PoopLauncherBlock extends BlockWithEntity {

    private static final VoxelShape SHAPE = createShape();

    public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;

    public PoopLauncherBlock(Settings settings) {
        super(settings);
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        if (placer != null) {
            if (world.getBlockEntity(pos) instanceof PoopLauncherBlockEntity poopLauncherBlockEntity) {
                poopLauncherBlockEntity.setOwner(placer.getUuid());
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new PoopLauncherBlockEntity(pos, state);
    }

    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, EntityRegistry.POOP_LAUNCHER, PoopLauncherBlockEntity::tick);
    }
