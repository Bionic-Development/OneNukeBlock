...
public class PoopLauncherModel extends Model {
    ...
    public PoopLauncherModel(ModelPart root) {
        super(RenderLayer::getEntityCutoutNoCull);

        this.root = root;
        this.bone = root.getChild("bone");
        this.poop = this.bone.getChild("poop");
        this.ground = this.bone.getChild("ground");
        this.launcher = this.bone.getChild("launcher");
        this.bottom = root.getChild("bottom");
    }

    public void setAngles(PoopLauncherBlockEntity poopLauncherBlockEntity, float chargeProgress, float animationProgress) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.poop.translate(new Vector3f(0, 0, 3.5f * (1 - chargeProgress)));

        if (poopLauncherBlockEntity.getShootState().getTimeRunning() > 1650f) {
            poopLauncherBlockEntity.getShootState().stop();
        }

        this.updateAnimation(poopLauncherBlockEntity.getShootState(), SHOOT, animationProgress);
    }
    ...
    public void renderPie(MatrixStack matrices, VertexConsumer vertexConsumer, int light, int overlay, float red, float green, float blue, float alpha) {
        ...
        this.root.rotate(matrices);
        this.bone.rotate(matrices);
        this.poop.render(matrices, vertexConsumer, light, overlay, red, green, blue, alpha);
        matrices.pop();
    }
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData ground = bone.addChild("ground", ModelPartBuilder.create().uv(0, 54).cuboid(-12.0F, -5.0F, 4.0F, 8.0F, 1.0F, 8.0F, new Dilation(0.0F)), ModelTransform.pivot(8.0F, 4.0F, -8.0F));

        ModelPartData poop = bone.addChild("poop", ModelPartBuilder.create().uv(7, 7).cuboid(8.5F, -17.0F, -18.3F, 1.0F, 1.0F, 1.0F, new Dilation(0.0F))
                .uv(0, 7).cuboid(7.5F, -18.0F, -17.3F, 2.0F, 2.0F, 1.0F, new Dilation(0.0F))
                .uv(0, 0).cuboid(6.5F, -19.0F, -16.3F, 4.0F, 4.0F, 2.0F, new Dilation(0.0F))
                .uv(0, 64).cuboid(5.5F, -20.0F, -14.3F, 7.0F, 7.0F, 2.0F, new Dilation(0.0F))
                .uv(33, 54).cuboid(3.5F, -22.0F, -12.3F, 10.0F, 10.0F, 2.0F, new Dilation(0.0F))
                .uv(49, 33).cuboid(2.5F, -23.0F, -10.3F, 12.0F, 12.0F, 3.0F, new Dilation(0.0F)), ModelTransform.pivot(-8.5F, 1.0F, -0.7F));
        ...
        ModelPartData legs = launcher.addChild("legs", ModelPartBuilder.create().uv(58, 54).cuboid(-11.0F, -11.0F, 5.0F, 6.0F, 6.0F, 6.0F, new Dilation(0.0F))
    ...
    public static class AnimationHelper {
        public static void animate(PoopLauncherModel model, Animation animation, long runningTime, float scale, Vector3f tempVec) {
