...
public class CyberWardenSecurityBrain {

    public static final int EMERGE_DURATION = MathHelper.ceil(133.59999f);

    private static final List<SensorType<? extends Sensor<? super CyberWardenSecurityEntity>>> SENSORS = List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR);
    ...
    public static void updateActivities(CyberWardenSecurityEntity warden) {
    ...
    public static Brain<?> create(CyberWardenSecurityEntity warden) {
        Brain.Profile<CyberWardenSecurityEntity> profile = Brain.createProfile(MEMORY_MODULES, SENSORS);
        NbtOps nbtOps = NbtOps.INSTANCE;
        Brain<CyberWardenSecurityEntity> brain = profile.deserialize(new Dynamic<>(nbtOps, nbtOps.createMap(ImmutableMap.of(nbtOps.createString("memories"), nbtOps.emptyMap()))));
        CyberWardenSecurityBrain.addCoreActivities(brain);
        CyberWardenSecurityBrain.addEmergeActivities(brain);
        CyberWardenSecurityBrain.addDigActivities(brain);
        CyberWardenSecurityBrain.addIdleActivities(brain);
        CyberWardenSecurityBrain.addRoarActivities(brain);
        CyberWardenSecurityBrain.addFightActivities(warden, brain);
        CyberWardenSecurityBrain.addInvestigateActivities(brain);
        CyberWardenSecurityBrain.addSniffActivities(brain);
    ...
    private static void addCoreActivities(Brain<CyberWardenSecurityEntity> brain) {
        brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new StayAboveWaterTask(0.8f),
                ...
                FindRoarTargetTask.create(CyberWardenSecurityBrain::getPreferredTarget)));
    }

    private static void addEmergeActivities(Brain<CyberWardenSecurityEntity> brain) {
    ...
    private static void addDigActivities(Brain<CyberWardenSecurityEntity> brain) {

    }

    private static void addIdleActivities(Brain<CyberWardenSecurityEntity> brain) {
    ...
    private static void addInvestigateActivities(Brain<CyberWardenSecurityEntity> brain) {
        brain.setTaskList(Activity.INVESTIGATE, 5, ImmutableList.of(FindRoarTargetTask.create(CyberWardenSecurityBrain::getPreferredTarget),
                WalkTowardsPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7f)), MemoryModuleType.DISTURBANCE_LOCATION);
    }

    private static void addSniffActivities(Brain<CyberWardenSecurityEntity> brain) {
        brain.setTaskList(Activity.SNIFF, 5, ImmutableList.of());
    }

    private static void addRoarActivities(Brain<CyberWardenSecurityEntity> brain) {
        brain.setTaskList(Activity.ROAR, 10, ImmutableList.of(new RoarTask()), MemoryModuleType.ROAR_TARGET);
    }

    private static void addFightActivities(CyberWardenSecurityEntity warden, Brain<CyberWardenSecurityEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(
                ForgetAttackTargetTask.create(entity -> !warden.isValidTarget(entity),
                        CyberWardenSecurityBrain::removeDeadSuspect, false),
                LookAtMobTask.create((LivingEntity entity) -> CyberWardenSecurityBrain.isTargeting(warden, entity),
                        (float) warden.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE)),
                create(1.2f)), MemoryModuleType.ATTACK_TARGET);
    }


    public static Task<MobEntity> create(float speed) {
        return create(entity -> Float.valueOf(speed));
    }
    ...
    public static Task<MobEntity> create(Function<LivingEntity, Float> speed) {
        return TaskTriggerer.task(context -> context.group(context.queryMemoryOptional(MemoryModuleType.WALK_TARGET), context.queryMemoryOptional(MemoryModuleType.LOOK_TARGET), context.queryMemoryValue(MemoryModuleType.ATTACK_TARGET), context.queryMemoryOptional(MemoryModuleType.VISIBLE_MOBS)).apply(context, (walkTarget, lookTarget, attackTarget, visibleMobs) -> (world, entity, time) -> {
            ...
            if (optional.isPresent() && optional.get().contains(livingEntity) && LookTargetUtil.isTargetWithinAttackRange(entity, livingEntity, 1)) {
                walkTarget.forget();
            } else {
                lookTarget.remember(new EntityLookTarget(livingEntity, true));
                walkTarget.remember(new WalkTarget(new EntityLookTarget(livingEntity, false), speed.apply(entity).floatValue(), 10));
            }
            return true;
        }));
    }

    private static boolean isTargeting(CyberWardenSecurityEntity warden, LivingEntity entity2) {
        return warden.getBrain().getOptionalMemory(MemoryModuleType.ATTACK_TARGET).filter(entity -> entity == entity2).isPresent();
    }

    private static void removeDeadSuspect(CyberWardenSecurityEntity warden, LivingEntity suspect) {
        if (!warden.isValidTarget(suspect)) {
            warden.removeSuspect(suspect);
        }
        CyberWardenSecurityBrain.resetDigCooldown(warden);
    }
