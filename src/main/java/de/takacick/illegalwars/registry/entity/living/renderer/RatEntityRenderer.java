...
import de.takacick.illegalwars.registry.entity.living.RatEntity;
import de.takacick.illegalwars.registry.entity.living.model.RatEntityModel;
...
public class RatEntityRenderer extends MobEntityRenderer<RatEntity, RatEntityModel<RatEntity>> {
    private static final Identifier TEXTURE = new Identifier(IllegalWars.MOD_ID, "textures/entity/rat/rat_brown.png");
    ...
    private static final Map<RatEntity.Variant, Identifier> TEXTURES = Util.make(Maps.newHashMap(), variants -> {
        for (RatEntity.Variant variant : RatEntity.Variant.values()) {
            variants.put(variant, new Identifier(IllegalWars.MOD_ID, String.format(Locale.ROOT, "textures/entity/rat/rat_%s.png", variant.getName())));
        }
    });
