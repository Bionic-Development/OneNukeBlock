...
public class IronManExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(ImagineAnything.MOD_ID, "ironman");
    ...
    public IronManExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public IronManExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        if (destructionType == net.minecraft.world.explosion.Explosion.DestructionType.BREAK) {
        ...
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            ...
            new IronManExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
            ServerPlayNetworking.send(serverPlayerEntity, IronManExplosionHandler.IDENTIFIER, packetByteBuf);
        }
    }
        ...
        public void affectWorld(boolean particles) {
            ...
            if (particles) {
                if (this.power < 2.0f || !bl) {
                    this.world.addParticle(ParticleRegistry.IRON_MAN_EXPLOSION, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                } else {
                    this.world.addParticle(ParticleRegistry.IRON_MAN_EXPLOSION_EMITTER, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                }
            }
            if (bl && !particles) {
