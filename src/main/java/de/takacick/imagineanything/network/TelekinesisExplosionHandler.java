...
public class TelekinesisExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(ImagineAnything.MOD_ID, "telekinesis");
    ...
    public TelekinesisExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public TelekinesisExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            ...
            new TelekinesisExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
            ServerPlayNetworking.send(serverPlayerEntity, TelekinesisExplosionHandler.IDENTIFIER, packetByteBuf);
        }
    }
        ...
        public void affectWorld(boolean particles) {
            ...
            if (particles) {
                if (this.power < 2.0f || !bl) {
                    this.world.addParticle(ParticleRegistry.TELEKINESIS_EXPLOSION, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                } else {
                    this.world.addParticle(ParticleRegistry.TELEKINESIS_EXPLOSION_EMITTER, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                }
            }
            if (bl && !particles) {
