package de.takacick.imagineanything.network;
...
public class ThanosExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(ImagineAnything.MOD_ID, "thanos");
    ...
    public ThanosExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public ThanosExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            ...
            new ThanosExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
            ServerPlayNetworking.send(serverPlayerEntity, ThanosExplosionHandler.IDENTIFIER, packetByteBuf);
        }
    }
    ...
    public static class Explosion extends net.minecraft.world.explosion.Explosion {
        ...
        public void collectBlocksAndDamageEntities() {
            ...
            for (int v = 0; v < list.size(); ++v) {
                ...
                double ac = (1.0 - w) * ab;
                entity.damage(this.getDamageSource(), (int) Math.min(((ac * ac + ac) / 2.0 * 7.0 * (double) q + 1.0), 6));
                double ad = ac;
        ...
        public void affectWorld(boolean particles) {
            ...
            if (particles) {
                if (this.power < 2.0f || !bl) {
                    this.world.addParticle(ParticleRegistry.THANOS_EXPLOSION, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                } else {
                    this.world.addParticle(ParticleRegistry.THANOS_EXPLOSION_EMITTER, this.x, this.y, this.z, 1.0, 0.0, 0.0);
                }
            }
            if (bl && !particles) {
