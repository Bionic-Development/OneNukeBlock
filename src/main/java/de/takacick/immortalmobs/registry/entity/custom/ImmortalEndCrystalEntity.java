...
public class ImmortalEndCrystalEntity
        extends Entity {
    private static final TrackedData<Optional<BlockPos>> BEAM_TARGET = DataTracker.registerData(ImmortalEndCrystalEntity.class, TrackedDataHandlerRegistry.OPTIONAL_BLOCK_POS);
    private static final TrackedData<Boolean> SHOW_BOTTOM = DataTracker.registerData(ImmortalEndCrystalEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    public int endCrystalAge;

    public ImmortalEndCrystalEntity(EntityType<? extends ImmortalEndCrystalEntity> entityType, World world) {
        super(entityType, world);
        this.intersectionChecked = true;
        this.endCrystalAge = this.random.nextInt(100000);
    }

    public ImmortalEndCrystalEntity(World world, double x, double y, double z) {
        this(EntityRegistry.IMMORTAL_END_CRYSTAL, world);
        this.setPosition(x, y, z);
    }

    public static ImmortalEndCrystalEntity create(EntityType<ImmortalEndCrystalEntity> entityType, World world) {
        return new ImmortalEndCrystalEntity(entityType, world);
    }
        ...
        if (!this.isRemoved() && !this.world.isClient) {
            ...
            if (!source.isExplosive()) {
                ImmortalFireworkExplosionHandler.createExplosion((ServerWorld) world, null, null, null,
                        getX(), getBodyY(0.5), getZ(), 6.0f, false,
