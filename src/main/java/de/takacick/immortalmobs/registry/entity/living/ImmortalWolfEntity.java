...
public class ImmortalWolfEntity
        extends TameableEntity
        implements Angerable, ImmortalEntity {
    private static final TrackedData<Boolean> BEGGING = DataTracker.registerData(ImmortalWolfEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Integer> COLLAR_COLOR = DataTracker.registerData(ImmortalWolfEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> ANGER_TIME = DataTracker.registerData(ImmortalWolfEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    public ImmortalWolfEntity(EntityType<? extends ImmortalWolfEntity> entityType, World world) {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(1, new ImmortalWolfEntity.WolfEscapeDangerGoal(1.5));
        this.goalSelector.add(2, new SitGoal(this));
        this.goalSelector.add(3, new ImmortalWolfEntity.AvoidLlamaGoal<>(this, LlamaEntity.class, 24.0f, 1.5, 1.5));
        ...
        this.targetSelector.add(7, new ActiveTargetGoal<>(this, ImmortalSkeletonEntity.class, false));
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(COLLAR_COLOR, DyeColor.PURPLE.getId());
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.isTamed() || !itemStack.isOf(ItemRegistry.IMMORTAL_PORKCHOP)) {
            player.sendMessage(Text.of("<§dImmortal §fWolf> *woof* *bark* *bark* *woof*"));
        }
        ...
        if (!itemStack.isOf(ItemRegistry.IMMORTAL_PORKCHOP)) return ActionResult.SUCCESS;

        player.sendMessage(Text.of("<§dImmortal §fWolf> I now love you- I mean, *woof*"));

        this.setOwner(player);
        this.navigation.stop();
        this.setTarget(null);
        this.setSitting(true);
        this.setCollarColor(DyeColor.PURPLE);
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        ...
        if (data.equals(COLLAR_COLOR)) {
            if (!world.isClient) {
                BionicUtils.sendEntityStatus((ServerWorld) world, this, ImmortalMobs.IDENTIFIER, 1);
            } else {
                float[] c = SheepEntity.getRgbColor(getCollarColor());
                Vec3f color = new Vec3f(c[0], c[1], c[2]);
                world.addParticle(new ImmortalWolfParticleEffect(color, this.getHeight()), this.getX(), this.getY(), this.getZ(), 0, -1, 0);
                world.addParticle(new ImmortalWolfParticleEffect(color, this.getHeight() + 0.2f), this.getX(), this.getY() - 0.2f, this.getZ(), 0, -1, 0);
                world.addParticle(new ImmortalWolfParticleEffect(color, this.getHeight() + 0.4f), this.getX(), this.getY() - 0.4f, this.getZ(), 0, -1, 0);
            }
        }
    ...
    @Override
    public ImmortalWolfEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        return null;
    }
    ...
    public boolean canBreedWith(AnimalEntity other) {
        ...
        if (!(other instanceof ImmortalWolfEntity)) {
            return false;
        }
        ImmortalWolfEntity wolfEntity = (ImmortalWolfEntity) other;
    ...
    public boolean canAttackWithOwner(LivingEntity target, LivingEntity owner) {
        ...
        if (target instanceof ImmortalWolfEntity) {
            ImmortalWolfEntity wolfEntity = (ImmortalWolfEntity) target;
    ...
    public static boolean canSpawn(EntityType<ImmortalWolfEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.getBlockState(pos.down()).isIn(BlockTags.WOLVES_SPAWNABLE_ON) && ImmortalWolfEntity.isLightLevelValidForNaturalSpawn(world, pos);
    }

    class WolfEscapeDangerGoal
            extends EscapeDangerGoal {
        public WolfEscapeDangerGoal(double speed) {
            super(ImmortalWolfEntity.this, speed);
        }
    ...
    class AvoidLlamaGoal<T extends LivingEntity>
        ...
        private final ImmortalWolfEntity wolf;

        public AvoidLlamaGoal(ImmortalWolfEntity wolf, Class<T> fleeFromType, float distance, double slowSpeed, double fastSpeed) {
        ...
        private boolean isScaredOf(LlamaEntity llama) {
            return llama.getStrength() >= ImmortalWolfEntity.this.random.nextInt(5);
        }

        @Override
        public void start() {
            ImmortalWolfEntity.this.setTarget(null);
            super.start();
        }
