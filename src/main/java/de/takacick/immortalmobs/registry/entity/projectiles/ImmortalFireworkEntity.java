...
public class ImmortalFireworkEntity extends ProjectileEntity implements FlyingItemEntity {
    private static final TrackedData<ItemStack> ITEM = DataTracker.registerData(ImmortalFireworkEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    private static final TrackedData<OptionalInt> SHOOTER_ENTITY_ID = DataTracker.registerData(ImmortalFireworkEntity.class, TrackedDataHandlerRegistry.OPTIONAL_INT);
    private static final TrackedData<Boolean> SHOT_AT_ANGLE = DataTracker.registerData(ImmortalFireworkEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private int life;
    ...
    public ImmortalFireworkEntity(EntityType<? extends ImmortalFireworkEntity> entityType, World world) {
        super(entityType, world);
    }

    public ImmortalFireworkEntity(World world, double x, double y, double z, ItemStack stack) {
        super(EntityRegistry.IMMORTAL_FIREWORK, world);
        this.life = 0;
    ...
    public ImmortalFireworkEntity(World world, @Nullable Entity entity, double x, double y, double z, ItemStack stack) {
        this(world, x, y, z, stack);
        this.setOwner(entity);
    }

    public ImmortalFireworkEntity(World world, ItemStack stack, LivingEntity shooter) {
    ...
    public ImmortalFireworkEntity(World world, ItemStack stack, double x, double y, double z, boolean shotAtAngle) {
        this(world, x, y, z, stack);
        this.dataTracker.set(SHOT_AT_ANGLE, shotAtAngle);
    }

    public ImmortalFireworkEntity(World world, ItemStack stack, Entity entity, double x, double y, double z, boolean shotAtAngle) {
        this(world, stack, x, y, z, shotAtAngle);
        this.setOwner(entity);
    }

    public static ImmortalFireworkEntity create(EntityType<ImmortalFireworkEntity> entityType, World world) {
        return new ImmortalFireworkEntity(entityType, world);
    }
        ...
        if (this.wasShotByEntity()) {
            ...
            if (this.shooter != null) {
                if (this.shooter.isFallFlying()) {
                    ...
                    vec3d3 = this.shooter.getHandPosOffset(ItemRegistry.IMMORTAL_FIREWORK);
                } else {
                    vec3d3 = Vec3d.ZERO;
                }
    ...
    private void explode() {
        if (wasShotAtAngle()) {
            ImmortalFireworkExplosionHandler.createExplosion((ServerWorld) world, this, DamageSource.thrownProjectile(this, getOwner()), null, getX(), getBodyY(0.5), getZ(), 10f, false, Explosion.DestructionType.DESTROY);
            return;
        }

        for (int i = 0; i < 8; i++) {
            ImmortalFireworkExplosionEntity immortalFireworkExplosionEntity = new ImmortalFireworkExplosionEntity(world, getOwner(), getX(), getY() - 0.4, getZ(), getStack());
            immortalFireworkExplosionEntity.setVelocity(random.nextGaussian() * 0.8, -1.8, random.nextGaussian() * 0.8);
            immortalFireworkExplosionEntity.life = 0;
            world.spawnEntity(immortalFireworkExplosionEntity);
        }
        ...
        if (status == EntityStatuses.EXPLODE_FIREWORK_CLIENT && this.world.isClient) {
            if (!this.hasExplosionEffects()) {
                ...
                if (wasShotAtAngle()) {
                    ...
                    ImmortalMobsClient.addFireworkParticle(world, getX(), getY() - 2, getZ(), vec3d.x, vec3d.y, vec3d.z, nbtCompound);

                    world.playSound(getX(), getY(), getZ(), SoundEvents.ENTITY_FIREWORK_ROCKET_LARGE_BLAST, getSoundCategory(), 1.0f, 1.0f, false);
                    world.addParticle(ParticleTypes.FLASH, getX(), getBodyY(0.5), getZ(), 0, -1, 0);
                } else {
                    ItemStack itemStack = this.dataTracker.get(ITEM);
                    NbtCompound nbtCompound = itemStack.isEmpty() ? null : itemStack.getSubNbt("Fireworks");
                    Vec3d vec3d = this.getVelocity();
                    ImmortalMobsClient.addFireworkParticle(world, this.getX(), this.getY(), this.getZ(), vec3d.x, vec3d.y, vec3d.z, nbtCompound);
                }
            }
        }
