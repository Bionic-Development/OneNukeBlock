...
public class ImmortalFireworksSparkParticle {
        ...
        public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
            if (!this.flicker || this.age < this.maxAge / 3 || (this.age + this.maxAge) / 3 % 2 == 0) {
                ...
                if (!this.trail && this.burst) {
                    ...
                    for (int i = 0; i < 1; i++) {
                        vertexConsumer.vertex(vec3fs[0].getX(), vec3fs[0].getY(), vec3fs[0].getZ()).color(0.4029412f, 0.14705883f, 0.5411765f, 0.1f).texture(m, o).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
                        vertexConsumer.vertex(vec3fs[1].getX(), vec3fs[1].getY(), vec3fs[1].getZ()).color(0.4029412f, 0.14705883f, 0.5411765f, 0.1f).texture(m, n).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
                        vertexConsumer.vertex(vec3fs[2].getX(), vec3fs[2].getY(), vec3fs[2].getZ()).color(0.4029412f, 0.14705883f, 0.5411765f, 0.1f).texture(l, n).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
                        vertexConsumer.vertex(vec3fs[3].getX(), vec3fs[3].getY(), vec3fs[3].getZ()).color(0.4029412f, 0.14705883f, 0.5411765f, 0.1f).texture(l, o).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
                    }
                }
    ...
    public static class FireworkParticle
        ...
        private void addExplosionParticle(double x, double y, double z, double velocityX, double velocityY, double velocityZ, int[] colors, int[] fadeColors, boolean trail, boolean flicker) {
            Explosion explosion = (Explosion) this.particleManager.addParticle(ParticleRegistry.IMMORTAL_FIREWORK, x, y, z, velocityX, velocityY, velocityZ);
