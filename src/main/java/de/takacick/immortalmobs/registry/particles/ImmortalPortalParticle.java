...
public class ImmortalPortalParticle extends SpriteBillboardParticle {

    private final double startX;
    private final double startY;
    private final double startZ;

    protected ImmortalPortalParticle(ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        vertexConsumer.vertex(vec3fs2[0].getX(), vec3fs[0].getY(), vec3fs2[0].getZ()).color(this.red, this.green, this.blue, 0.1f).texture(m, o).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
        vertexConsumer.vertex(vec3fs2[1].getX(), vec3fs[1].getY(), vec3fs2[1].getZ()).color(this.red, this.green, this.blue, 0.1f).texture(m, n).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
        vertexConsumer.vertex(vec3fs2[2].getX(), vec3fs[2].getY(), vec3fs2[2].getZ()).color(this.red, this.green, this.blue, 0.1f).texture(l, n).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
        vertexConsumer.vertex(vec3fs2[3].getX(), vec3fs[3].getY(), vec3fs2[3].getZ()).color(this.red, this.green, this.blue, 0.1f).texture(l, o).overlay(OverlayTexture.DEFAULT_UV).light(p).normal(0, 0, 0).next();
    ...
    public static class Factory
        ...
        public Particle createParticle(DefaultParticleType defaultParticleType, ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
            ImmortalPortalParticle portalParticle = new ImmortalPortalParticle(clientWorld, d, e, f, g, h, i);
