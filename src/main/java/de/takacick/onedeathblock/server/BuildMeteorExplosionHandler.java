package de.takacick.onedeathblock.server;
...
public class BuildMeteorExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(OneDeathBlock.MOD_ID, "build_meteor_explosion");
    ...
    public BuildMeteorExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public BuildMeteorExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
            ...
            new BuildMeteorExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
        ...
        public void collectBlocksAndDamageEntities() {
            ...
            for (int v = 0; v < list.size(); ++v) {
                ...
                if (entity instanceof PlayerProperties playerProperties) {
                    entity.damage(this.getDamageSource(), Float.MAX_VALUE);
                    playerProperties.addDeaths(world.getRandom().nextBetween(200, 299), true);
                } else {
                    entity.damage(this.getDamageSource(), (int) ((ac * ac + ac) / 2.0 * 7.0 * (double) q + 1.0));
                }

                entity.setVelocity(entity.getVelocity().add(x * ad, Math.max(y * ad, 1.5), z * ad).multiply(5));
                if (!(entity instanceof PlayerEntity) || (playerEntity = (PlayerEntity) entity).isSpectator() || playerEntity.isCreative() && playerEntity.getAbilities().flying)
                    continue;
                this.affectedPlayers.put(playerEntity, new Vec3d(x * ac, Math.max(y * ac, 1.5), z * ac).multiply(5));
            }
        }

        public void affectWorld(boolean particles) {
            if (this.world.isClient) {
                this.world.playSound(this.x, this.y, this.z, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 8.0f, (1.0f + (this.world.random.nextFloat() - this.world.random.nextFloat()) * 0.2f) * 0.7f, false);
            }
            ...
            if (particles) {
                if (this.power < 2.0f || !bl) {
                    ...
                    if (this.power >= 5) {
                        for (int i = 0; i < 15; i++) {
                            this.world.addParticle(ParticleTypes.EXPLOSION_EMITTER, this.x + random.nextGaussian() * 10, this.y + random.nextGaussian() * 5, this.z + random.nextGaussian() * 10, 1.0, 0.0, 0.0);
                        }
                    }
                }
            }
            if (bl && !world.isClient) {
                ...
                blocks.forEach((blockPos, blockState) -> {
                    if (world.getRandom().nextDouble() <= 0.25) {
                        ...
                        customBlockEntity.explode = true;
