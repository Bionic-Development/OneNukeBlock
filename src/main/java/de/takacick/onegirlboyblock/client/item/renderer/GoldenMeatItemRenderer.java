...
public class GoldenMeatItemRenderer extends ItemRenderer {

    public GoldenMeatItemRenderer() {
    ...
    public void render(ItemStack itemStack, @Nullable LivingEntity livingEntity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, ModelTransformationMode modelTransformationMode, int light, int uv) {
        ...
        if (helper.isTicking()) {
            ...
            for (int i = 0; i < 2; i++) {
                Vec3d vector = new Vec3d(random.nextGaussian(), random.nextGaussian(), random.nextGaussian()).normalize().multiply(5f / 16f, 5f / 16f, 5f / 16f);

                Vec3d offset = new Vec3d(0f, 14f / 16f, 0f).add(vector);

                Particle particle = client.particleManager.createParticle(ParticleRegistry.GOLD_NUGGET, offset.getX(), offset.getY(), offset.getZ(), vector.getX(), vector.getY(), vector.getZ());
