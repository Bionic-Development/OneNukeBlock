...
public class StarMinerItemRenderer extends ItemRenderer {

    public StarMinerItemRenderer() {
    ...
    public void render(ItemStack itemStack, @Nullable LivingEntity livingEntity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, ModelTransformationMode modelTransformationMode, int light, int uv) {
        ...
        if (helper.isTicking()) {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                Vec3d offset = new Vec3d(0f, 14f / 16f, 0f).add(vector.multiply(7f / 16, 1f / 16, 7f / 16f));

                Particle particle = client.particleManager.createParticle(ParticleRegistry.STAR_MINER_GLITTER, offset.getX(), offset.getY(), offset.getZ(), vector.getX() * 0.05, vector.getY() * 0.05, vector.getZ() * 0.05);
