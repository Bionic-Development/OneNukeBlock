...
public class GrowParticle
    ...
    private GrowParticle(ClientWorld world, int owner, double x, double y, double z, double velocityX, double velocityY, double velocityZ, SpriteProvider spriteProvider) {
        ...
        Vector3f color = Vec3d.unpackRgb(0xFFEC4F).toVector3f();
        setColor(color.x(), color.y(), color.z());

        if (owner >= 0) {
            this.ownerEntity = world.getEntityById(owner);
            if (this.ownerEntity != null) {
                this.offset = this.ownerEntity.getPos().subtract(new Vec3d(x, y, z));
                this.prevOffset = this.offset;
                this.maxAge = (int) (Math.max((float) (this.ownerEntity.getBodyY(1) - this.y), 0f) * 4f) + world.getRandom().nextInt(6);

                ScaleData scaleData = ScaleTypes.BASE.getScaleData(this.ownerEntity);
                this.scale *= scaleData.getScale() * 0.5f;
            }
        }
    }
    ...
    public static class Factory
        ...
        public Particle createParticle(EntityParticleEffect particleEffect, ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
            return new GrowParticle(clientWorld, particleEffect.owner(), d, e, f, g, h, i, this.spriteProvider);
        }
    }
}
