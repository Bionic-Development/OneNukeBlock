...
public class FollowBuffLoveGoal
extends Goal {
    protected final BuffChadVillagerEntity mob;
    ...
    public FollowBuffLoveGoal(BuffChadVillagerEntity mob, double speed, boolean pauseWhenMobIdle) {
        this.mob = mob;
        this.speed = speed;
    ...
    public boolean canStart() {
        ...
        this.path = this.mob.getNavigation().findPathTo(livingEntity, 3);
        if (this.path != null) {
            return true;
        }
    ...
    public void tick() {
        ...
        if ((this.pauseWhenMobIdle || this.mob.getVisibilityCache().canSee(livingEntity)) && this.updateCountdownTicks <= 0 && (this.targetX == 0.0 && this.targetY == 0.0 && this.targetZ == 0.0 || livingEntity.squaredDistanceTo(this.targetX, this.targetY, this.targetZ) >= 1.0 || this.mob.getRandom().nextFloat() < 0.05f)) {
            ...
            if (!startMovingTo(livingEntity, this.speed)) {
    ...
    public boolean startMovingTo(Entity entity, double speed) {
        Path path = this.mob.getNavigation().findPathTo(entity, 3);
        return path != null && this.mob.getNavigation().startMovingAlong(path, speed);
    }
