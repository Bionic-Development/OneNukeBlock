package de.takacick.onegirlfriendblock.registry.entity.living.ai;
...
public class ZukoHumanoidBrain {

    public static final int EMERGE_DURATION = MathHelper.ceil(133.59999f);

    private static final List<SensorType<? extends Sensor<? super ZukoHumanoidEntity>>> SENSORS = List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR);
    ...
    public static void updateActivities(ZukoHumanoidEntity warden) {
    ...
    public static Brain<?> create(ZukoHumanoidEntity warden) {
        Brain.Profile<ZukoHumanoidEntity> profile = Brain.createProfile(MEMORY_MODULES, SENSORS);
        NbtOps nbtOps = NbtOps.INSTANCE;
        Brain<ZukoHumanoidEntity> brain = profile.deserialize(new Dynamic<>(nbtOps, nbtOps.createMap(ImmutableMap.of(nbtOps.createString("memories"), nbtOps.emptyMap()))));
        ZukoHumanoidBrain.addCoreActivities(brain);
        ZukoHumanoidBrain.addEmergeActivities(brain);
        ZukoHumanoidBrain.addDigActivities(brain);
        ZukoHumanoidBrain.addIdleActivities(brain);
        ZukoHumanoidBrain.addRoarActivities(brain);
        ZukoHumanoidBrain.addFightActivities(warden, brain);
        ZukoHumanoidBrain.addInvestigateActivities(brain);
        ZukoHumanoidBrain.addSniffActivities(brain);
    ...
    private static void addCoreActivities(Brain<ZukoHumanoidEntity> brain) {
        brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new StayAboveWaterTask(0.8f),
                ...
                FindRoarTargetTask.create(ZukoHumanoidBrain::getPreferredTarget)));
    }

    private static void addEmergeActivities(Brain<ZukoHumanoidEntity> brain) {
    ...
    private static void addDigActivities(Brain<ZukoHumanoidEntity> brain) {

    }

    private static void addIdleActivities(Brain<ZukoHumanoidEntity> brain) {
    ...
    private static void addInvestigateActivities(Brain<ZukoHumanoidEntity> brain) {
        brain.setTaskList(Activity.INVESTIGATE, 5, ImmutableList.of(FindRoarTargetTask.create(ZukoHumanoidBrain::getPreferredTarget),
                WalkTowardsPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7f)), MemoryModuleType.DISTURBANCE_LOCATION);
    }

    private static void addSniffActivities(Brain<ZukoHumanoidEntity> brain) {
        brain.setTaskList(Activity.SNIFF, 5, ImmutableList.of());
    }

    private static void addRoarActivities(Brain<ZukoHumanoidEntity> brain) {
        brain.setTaskList(Activity.ROAR, 10, ImmutableList.of(new RoarTask()), MemoryModuleType.ROAR_TARGET);
    }

    private static void addFightActivities(ZukoHumanoidEntity warden, Brain<ZukoHumanoidEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(
                ForgetAttackTargetTask.create(entity -> !warden.isValidTarget(entity),
                        ZukoHumanoidBrain::removeDeadSuspect, false),
                LookAtMobTask.create((LivingEntity entity) -> ZukoHumanoidBrain.isTargeting(warden, entity),
    ...
    private static boolean isTargeting(ZukoHumanoidEntity warden, LivingEntity entity2) {
        return warden.getBrain().getOptionalMemory(MemoryModuleType.ATTACK_TARGET).filter(entity -> entity == entity2).isPresent();
    }

    private static void removeDeadSuspect(ZukoHumanoidEntity warden, LivingEntity suspect) {
        if (!warden.isValidTarget(suspect)) {
            warden.removeSuspect(suspect);
        }
        ZukoHumanoidBrain.resetDigCooldown(warden);
    }
