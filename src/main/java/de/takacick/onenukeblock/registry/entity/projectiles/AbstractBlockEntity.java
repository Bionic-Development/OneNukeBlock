...
public abstract class AbstractBlockEntity extends Entity {
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(SIZE, getStartSize());
        builder.add(PROGRESS, 0f);
    }

    public void initializeTarget(Vec3d end) {
        setAttached(AttachmentTypes.BEZIER_CURVE, new BezierCurve(getPos(), end, false));
    }
    ...
    public BezierCurve getBezierCurve() {
        return getAttachedOrCreate(AttachmentTypes.BEZIER_CURVE);
    }
    ...
    protected void readCustomDataFromNbt(NbtCompound nbt) {
        if (nbt.contains("fromBlockPos", NbtElement.COMPOUND_TYPE)) {
            setFromBlockPos(NbtHelper.toBlockPos(nbt, "fromBlockPos").orElse(BlockPos.ORIGIN));
        }
