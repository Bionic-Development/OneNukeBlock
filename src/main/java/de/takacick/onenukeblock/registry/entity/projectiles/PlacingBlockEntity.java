...
import de.takacick.onenukeblock.utils.worldedit.WorldChunkUtils;
...
public class PlacingBlockEntity extends AbstractBlockEntity {
    ...
    public void initializeTarget(Vec3d end) {
        setAttached(AttachmentTypes.BEZIER_CURVE, new BezierCurve(getPos(), end, true));
    }
        ...
        if (this.progress >= 0.75f) {
            ...
            if (!getWorld().isClient) {
                if (this.progress >= 1.8f && this.size >= 1f) {
                    ...
                    if (WorldChunkUtils.setBlockState(getWorld(), targetPos, getBlockState(), false) == null) {
                        this.discard();
                    }

                    BlockEntity blockEntity;
                    if (this.nbtCompound != null && (blockEntity = getWorld().getBlockEntity(targetPos)) != null) {
                        blockEntity.createNbt(this.getWorld().getServer().getRegistryManager());
                        blockEntity.read(this.nbtCompound, this.getWorld().getServer().getRegistryManager());
                    }
                }
