...
public abstract class SpriteAAParticle extends SpriteBillboardParticle {
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        vertexConsumer.vertex(vec3fs[0].x(), vec3fs[0].y(), vec3fs[0].z()).texture(getMaxU(), getMaxV()).color(this.red, this.green, this.blue, this.alpha).light(n);
        vertexConsumer.vertex(vec3fs[1].x(), vec3fs[1].y(), vec3fs[1].z()).texture(getMaxU(), getMinV()).color(this.red, this.green, this.blue, this.alpha).light(n);
        vertexConsumer.vertex(vec3fs[2].x(), vec3fs[2].y(), vec3fs[2].z()).texture(getMinU(), getMinV()).color(this.red, this.green, this.blue, this.alpha).light(n);
        vertexConsumer.vertex(vec3fs[3].x(), vec3fs[3].y(), vec3fs[3].z()).texture(getMinU(), getMaxV()).color(this.red, this.green, this.blue, this.alpha).light(n);
    }
}