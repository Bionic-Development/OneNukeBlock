    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        for (int y = 1, vi = 0; y < (int) ts + 1; y++, vi++) {
            for (int x = 1; x < (int) ts + 1; x++, vi++) {
                ...
                if (render) {
                    //top
                    vertexConsumer.vertex(modVerts[0].x(), modVerts[0].y(), modVerts[0].z()).texture(uvs.get(vi).x, uvs.get(vi).y).color(this.red, this.green, this.blue, this.alpha).light(n);
                    vertexConsumer.vertex(modVerts[1].x(), modVerts[1].y(), modVerts[1].z()).texture(uvs.get((int) (vi + ts + 1)).x, uvs.get((int) (vi + ts + 1)).y).color(this.red, this.green, this.blue, this.alpha).light(n);
                    vertexConsumer.vertex(modVerts[2].x(), modVerts[2].y(), modVerts[2].z()).texture(uvs.get((int) (vi + ts + 2)).x, uvs.get((int) (vi + ts + 2)).y).color(this.red, this.green, this.blue, this.alpha).light(n);
                    vertexConsumer.vertex(modVerts[3].x(), modVerts[3].y(), modVerts[3].z()).texture(uvs.get(vi + 1).x, uvs.get(vi + 1).y).color(this.red, this.green, this.blue, this.alpha).light(n);
                    //bottom
                    vertexConsumer.vertex(modVerts[3].x(), modVerts[3].y(), modVerts[3].z()).texture(uvs.get(vi + 1).x, uvs.get(vi + 1).y).color(this.red, this.green, this.blue, this.alpha).light(n);
                    vertexConsumer.vertex(modVerts[2].x(), modVerts[2].y(), modVerts[2].z()).texture(uvs.get((int) (vi + ts + 2)).x, uvs.get((int) (vi + ts + 2)).y).color(this.red, this.green, this.blue, this.alpha).light(n);
                    vertexConsumer.vertex(modVerts[1].x(), modVerts[1].y(), modVerts[1].z()).texture(uvs.get((int) (vi + ts + 1)).x, uvs.get((int) (vi + ts + 1)).y).color(this.red, this.green, this.blue, this.alpha).light(n);
