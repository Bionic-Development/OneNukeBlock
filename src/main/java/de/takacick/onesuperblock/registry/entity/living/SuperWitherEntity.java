...
public class SuperWitherEntity extends HostileEntity implements SkinOverlayOwner, RangedAttackMob {
    private static final TrackedData<Integer> TRACKED_ENTITY_ID_1 = DataTracker.registerData(SuperWitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> TRACKED_ENTITY_ID_2 = DataTracker.registerData(SuperWitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> TRACKED_ENTITY_ID_3 = DataTracker.registerData(SuperWitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private static final TrackedData<Integer> INVUL_TIMER = DataTracker.registerData(SuperWitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private final ServerBossBar bossBar = (ServerBossBar) new ServerBossBar(this.getDisplayName(), OneSuperBlock.RAINBOW_BAR, BossBar.Style.PROGRESS).setDarkenSky(true);
    ...
    public SuperWitherEntity(EntityType<? extends SuperWitherEntity> entityType, World world) {
        ...
        for (i = 1; i < 3; ++i) {
            ...
            if (this.world.getDifficulty() == Difficulty.NORMAL || this.world.getDifficulty() == Difficulty.HARD) {
                ...
                if (n2 > 15) {
                    ...
                    this.shootSkullAt(i + 1, d, e, g, false);
                    this.chargedSkullCooldowns[i - 1] = 0;
                }
            }
            if ((destructionType = this.getTrackedEntityId(i)) > 0) {
                LivingEntity l = (LivingEntity) this.world.getEntityById(destructionType);
                if (l == null || !this.canTarget(l) || this.squaredDistanceTo(l) > 900.0 || !this.canSee(l)) {
                    this.setTrackedEntityId(i, 0);
                    continue;
                }
                this.shootSkullAt(i + 1, l);
    ...
    private void shootSkullAt(int headIndex, double targetX, double targetY, double targetZ, boolean charged) {
        ...
        double h = targetY - e;
        double i = targetZ - f;
        SuperWitherSkullEntity superWitherSkullEntity = new SuperWitherSkullEntity(this.world, this, g, h, i);
        superWitherSkullEntity.setOwner(this);
        if (charged) {
            superWitherSkullEntity.setCharged(true);
        }
        superWitherSkullEntity.setPos(d, e, f);
        this.world.spawnEntity(superWitherSkullEntity);
    }
    ...
    public static DefaultAttributeContainer.Builder createWitherAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.GENERIC_MAX_HEALTH, 1000.0).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.6f).add(EntityAttributes.GENERIC_FOLLOW_RANGE, 40.0).add(EntityAttributes.GENERIC_ARMOR, 4.0);
    }
    ...
    public float getHeadYaw(int headIndex, float tickDelta) {
        return MathHelper.lerp(tickDelta, this.prevSideHeadYaws[headIndex], this.sideHeadYaws[headIndex]);
    }
    ...
    public float getHeadPitch(int headIndex, float tickDelta) {
        return MathHelper.lerp(tickDelta, this.prevSideHeadPitches[headIndex], this.sideHeadPitches[headIndex]);
    }
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!damageSource.isOutOfWorld() && !this.world.isClient) {
            BionicUtils.sendEntityStatus((ServerWorld) this.world, this, OneSuperBlock.IDENTIFIER, 5);
        }
    ...
    class DescendAtHalfHealthGoal extends Goal {
        ...
        public boolean canStart() {
            return SuperWitherEntity.this.getInvulnerableTimer() > 0;
        }
    }
