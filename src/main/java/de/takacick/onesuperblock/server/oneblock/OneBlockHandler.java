package de.takacick.onesuperblock.server.oneblock;
...
public class OneBlockHandler {
    ...
    private final Block oneBlock = ItemRegistry.SUPER_BLOCK;
    ...
    public OneBlockHandler() {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            LiteralCommandNode<ServerCommandSource> rootNode = CommandManager.literal("onesuperblock")
                    .requires(source -> {
                        ...
                        playerEntity.openHandledScreen(new SimpleNamedScreenHandlerFactory((syncId, inventory, player) -> DropsScreenHandler.createGeneric9x6(syncId, inventory, simpleInventory), Text.of("OneSuperBlock Drops: ")));
                        return 1;
                            ...
                            .requires(source -> source.hasPermissionLevel(2)).then(CommandManager.argument("pos", BlockPosArgumentType.blockPos()).executes(context -> {
                                ...
                                context.getSource().sendFeedback(Text.literal("Set OneSuperBlock to " + blockPos.getX() + ", " + blockPos.getY() + ", " + blockPos.getZ()), true);
                            ...
                            .executes(context -> {
                                ...
                                if (world == null || blockPos == null) {
                                    context.getSource().sendFeedback(Text.literal("§cCouldn't find OneSuperBlock"), false);
                                    return 0;
                                }
                                ...
                                context.getSource().sendFeedback(Text.literal("Teleport to OneSuperBlock"), true);
                            ...
                            .then(CommandManager.argument("entity", EntitySummonArgumentType.entitySummon()).suggests(SuggestionProviders.SUMMONABLE_ENTITIES)
                                    .executes((context) -> {
                                        ...
                                        context.getSource().sendFeedback(Text.of("Set next OneSuperBlock entity: " + Text.translatable(nextEntity.getTranslationKey()).getString()), true);
                                        return 1;
    ...
    private static void dropStack(World world, Supplier<ItemEntity> itemEntitySupplier, ItemStack stack) {
        ...
        ((ItemProperties) itemEntity).setRainbow(true);
