...
public class CopperHopperBlockEntity
    ...
    public CopperHopperBlockEntity(BlockPos pos, BlockState state) {
        super(EntityRegistry.COPPER_HOPPER, pos, state);
    }
    ...
    public static void onEntityCollided(World world, BlockPos pos, BlockState state, Entity entity, CopperHopperBlockEntity blockEntity) {
        if (entity instanceof ItemEntity && VoxelShapes.matchesAnywhere(VoxelShapes.cuboid(entity.getBoundingBox().offset(-pos.getX(), -pos.getY(), -pos.getZ())), blockEntity.getInputAreaShape(), BooleanBiFunction.AND)) {
            CopperHopperBlockEntity.insertAndExtract(world, pos, state, blockEntity, () -> CopperHopperBlockEntity.extract(blockEntity, (ItemEntity)entity));
        }
    }
