...
import de.takacick.secretgirlbase.registry.block.BubbleGumLauncherBlock;
...
public class BubbleGumLauncherBlockEntity extends BlockEntity {
    ...
    private float yaw = getCachedState().get(BubbleGumLauncherBlock.FACING).asRotation();
    private float pitch = 0;
    private float prevYaw = getCachedState().get(BubbleGumLauncherBlock.FACING).asRotation();
    private float prevPitch = 0;

    public BubbleGumLauncherBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(EntityRegistry.BUBBLE_GUM_LAUNCHER, blockPos, blockState);
    }

    public static void tick(World world, BlockPos pos, BlockState state, BubbleGumLauncherBlockEntity blockEntity) {
        blockEntity.age++;
        if (!world.isClient) {
            Vec3d vec3d = blockEntity.getHorizontalRotationVec().normalize().multiply(0.56);

            Vec3d center = Vec3d.ofBottomCenter(pos).add(vec3d).add(0, 1.06875, 0);
            ...
            LivingEntity target = world.getClosestEntity(LivingEntity.class,
                    TargetPredicate.createAttackable().setPredicate(livingEntity ->
                            livingEntity instanceof PlayerEntity
                                    && blockEntity.canSee(center.add(0, 0.2, 0), livingEntity)
                                    && !((livingEntity.getUuid().equals(blockEntity.owner)
                                    || (livingEntity instanceof Tameable tameable
                                    && (blockEntity.owner != null && blockEntity.owner.equals(tameable.getOwnerUuid()))))
            ...
            if (target != null) {
                ...
                if (blockEntity.lookAt(pos, target.getPos().add(0, target.getHeight() * 0.7, 0))
                    ...
                    world.playSound(null, pos, SoundEvents.BLOCK_DECORATED_POT_INSERT, SoundCategory.BLOCKS, 1f, 1.2f);
                    world.playSound(null, pos, SoundEvents.BLOCK_HONEY_BLOCK_SLIDE, SoundCategory.BLOCKS, 1f, 1.2f);

                    BubbleGumEntity bubbleGumEntity = new BubbleGumEntity(world, center.getX(), center.getY(), center.getZ(), null);
                    setVelocity(bubbleGumEntity, blockEntity.pitch, blockEntity.yaw, 0, 1.8f, 1f);
                    bubbleGumEntity.setPosition(center.getX(), center.getY(), center.getZ());
                    world.spawnEntity(bubbleGumEntity);
                    blockEntity.charge = 0;
                    blockEntity.cooldown = world.getRandom().nextBetween(7, 10);
                    world.syncWorldEvent(9412934, pos, 5);
                }
    ...
    public static void setVelocity(ProjectileEntity projectileEntity, float pitch, float yaw, float roll, float speed, float divergence) {
