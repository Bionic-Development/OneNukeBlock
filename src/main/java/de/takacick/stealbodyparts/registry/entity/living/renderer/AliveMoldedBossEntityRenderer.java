...
import de.takacick.stealbodyparts.registry.entity.custom.renderer.LightningFeatureRenderer;
...
public class AliveMoldedBossEntityRenderer extends MobEntityRenderer<AliveMoldingBodyEntity, AliveMoldingBodyEntityModel<AliveMoldingBodyEntity>> {

    private static final Identifier TEXTURE = new Identifier(StealBodyParts.MOD_ID, "textures/entity/alive_molded_boss.png");

    public AliveMoldedBossEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx, new AliveMoldingBodyEntityModel<>(AliveMoldingBodyEntityModel.getTexturedModelData().createModel()), 0.6f);

        this.addFeature(new AliveMoldingBodyFeatureRenderer<>(this));
        this.addFeature(new LightningFeatureRenderer<>(this));
    }

    @Override
    protected void scale(AliveMoldingBodyEntity aliveMoldingBodyEntity, MatrixStack matrices, float amount) {
        matrices.scale(0.9375f * 1.9f, 0.9375f * 1.9f, 0.9375f * 1.9f);
