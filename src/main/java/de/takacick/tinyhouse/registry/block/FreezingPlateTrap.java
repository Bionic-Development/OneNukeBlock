...
public class FreezingPlateTrap extends WeightedPressurePlateBlock {
    private static final int MAX_CHARGE = 10;
    public static final IntProperty CHARGE = IntProperty.of("charge", 0, MAX_CHARGE);

    private final BlockSetType blockSetType;

    public FreezingPlateTrap(Settings settings, BlockSetType blockSetType) {
        ...
        if (this.getRedstoneOutput(newState) > 1 && this.getCharge(newState) > 0) {
            ...
            world.getEntitiesByClass(Entity.class, BOX.offset(pos), EntityPredicates.EXCEPT_SPECTATOR.and(entity -> !entity.canAvoidTraps())).forEach(entity -> {
                if (entity instanceof LivingProperties livingProperties) {
                    ...
                    livingProperties.setBurning(false);
                    livingProperties.setFrozenBody(100);
                    entity.setFrozenTicks(300);
                }
            });

            world.syncWorldEvent(813923, pos, 3);
        }
    ...
    private void updatePlateState(@Nullable Entity entity, World world, BlockPos pos, BlockState state, int output) {
        ...
        if (i == 15 && output != i) {
            world.playSound(null, pos, ParticleRegistry.ICE_FREEZE, SoundCategory.BLOCKS);
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        ...
        tooltip.add(Text.of("§bFreezes §eanything at a single step!"));
