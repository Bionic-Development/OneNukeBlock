package de.takacick.tinyhouse.server.explosion;
...
public class ChickenExplosionHandler {

    public static final Identifier IDENTIFIER = new Identifier(TinyHouse.MOD_ID, "chicken_explosion");
    ...
    public ChickenExplosionHandler(double x, double y, double z, float radius, List<BlockPos> affectedBlocks, @Nullable Vec3d playerVelocity) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.radius = radius;
    ...
    public ChickenExplosionHandler(PacketByteBuf buf) {
    ...
    public static void createExplosion(ServerWorld world, @Nullable Entity entity, @Nullable DamageSource damageSource, @Nullable ExplosionBehavior behavior, double x, double y, double z, float power, boolean createFire, net.minecraft.world.explosion.Explosion.DestructionType destructionType) {
        ...
        if (destructionType != net.minecraft.world.explosion.Explosion.DestructionType.KEEP) {
            for (ServerPlayerEntity serverPlayerEntity : world.getPlayers()) {
                ...
                new ChickenExplosionHandler(x, y, z, power, explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)).write(packetByteBuf);
    ...
    public static class Explosion extends net.minecraft.world.explosion.Explosion {
        ...
        public void affectWorld(boolean particles) {
            ...
            if (this.world.isClient) {
                this.world.playSound(this.x, this.y, this.z, SoundEvents.ENTITY_CHICKEN_DEATH, SoundCategory.BLOCKS, 3.0f, (1.4f + (this.world.random.nextFloat() - this.world.random.nextFloat()) * 0.2f) * 0.7f, false);
            ...
            if (particles) {
                for (int i = 0; i < this.power * 10; i++) {
                    ...
                    this.world.addParticle(ParticleTypes.SMOKE, true, this.x + d, this.y + e, this.z + f, d * 1, e * 1, f * 1);
                }
                for (int i = 0; i < this.power * 10; i++) {
                    double d = random.nextGaussian();
                    double e = random.nextGaussian();
                    ...
                    this.world.addParticle(ParticleRegistry.FEATHER, true, this.x + d, this.y + e, this.z + f, d * 1, e * 1, f * 1);
                }
            ...
            if (bl) {
                ...
                int drops = world.getRandom().nextBetween(1, 4);
                ...
                for (int i = 0; i < drops && !this.affectedBlocks.isEmpty(); i++) {
                    ...
                    Vec3d velocity = pos.subtract(new Vec3d(x + 0.5, y + 0.5, z + 0.5)).normalize();
                    ItemEntity itemEntity = new ItemEntity(world, pos.getX(), pos.getY(), pos.getZ(), Items.FEATHER.getDefaultStack(), velocity.getX(), velocity.getY(), velocity.getZ());
                ...
                for (BlockPos blockPos : this.affectedBlocks) {
                    ...
                    if (block.shouldDropItemsOnExplosion(this) && (world = this.world) instanceof ServerWorld) {
                        if (world.getRandom().nextDouble() <= 0.25) {
                            world.setBlockState(blockPos, (world.getRandom().nextDouble() <= 0.6 ? Blocks.EMERALD_ORE : Blocks.EMERALD_BLOCK).getDefaultState());
                        }
                    }
                    this.world.getProfiler().pop();
                }
