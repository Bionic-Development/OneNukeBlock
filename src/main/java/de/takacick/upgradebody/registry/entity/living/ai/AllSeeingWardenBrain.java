...
public class AllSeeingWardenBrain {

    public static final int EMERGE_DURATION = MathHelper.ceil(133.59999f);

    private static final List<SensorType<? extends Sensor<? super AllSeeingWardenEntity>>> SENSORS = List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR);
    ...
    public static void updateActivities(AllSeeingWardenEntity warden) {
    ...
    public static Brain<?> create(AllSeeingWardenEntity warden) {
        Brain.Profile<AllSeeingWardenEntity> profile = Brain.createProfile(MEMORY_MODULES, SENSORS);
        NbtOps nbtOps = NbtOps.INSTANCE;
        Brain<AllSeeingWardenEntity> brain = profile.deserialize(new Dynamic<>(nbtOps, nbtOps.createMap(ImmutableMap.of(nbtOps.createString("memories"), nbtOps.emptyMap()))));
        AllSeeingWardenBrain.addCoreActivities(brain);
        AllSeeingWardenBrain.addEmergeActivities(brain);
        AllSeeingWardenBrain.addDigActivities(brain);
        AllSeeingWardenBrain.addIdleActivities(brain);
        AllSeeingWardenBrain.addRoarActivities(brain);
        AllSeeingWardenBrain.addFightActivities(warden, brain);
        AllSeeingWardenBrain.addInvestigateActivities(brain);
        AllSeeingWardenBrain.addSniffActivities(brain);
    ...
    private static void addCoreActivities(Brain<AllSeeingWardenEntity> brain) {
        brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new StayAboveWaterTask(0.8f),
                ...
                FindRoarTargetTask.create(AllSeeingWardenBrain::getPreferredTarget)));
    }

    private static void addEmergeActivities(Brain<AllSeeingWardenEntity> brain) {
    ...
    private static void addDigActivities(Brain<AllSeeingWardenEntity> brain) {

    }

    private static void addIdleActivities(Brain<AllSeeingWardenEntity> brain) {
    ...
    private static void addInvestigateActivities(Brain<AllSeeingWardenEntity> brain) {
        brain.setTaskList(Activity.INVESTIGATE, 5, ImmutableList.of(FindRoarTargetTask.create(AllSeeingWardenBrain::getPreferredTarget),
                WalkTowardsPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7f)), MemoryModuleType.DISTURBANCE_LOCATION);
    }

    private static void addSniffActivities(Brain<AllSeeingWardenEntity> brain) {
        brain.setTaskList(Activity.SNIFF, 5, ImmutableList.of());
    }

    private static void addRoarActivities(Brain<AllSeeingWardenEntity> brain) {
        brain.setTaskList(Activity.ROAR, 10, ImmutableList.of(new RoarTask()), MemoryModuleType.ROAR_TARGET);
    }

    private static void addFightActivities(AllSeeingWardenEntity warden, Brain<AllSeeingWardenEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(
                ForgetAttackTargetTask.create(entity -> !warden.isValidTarget(entity),
                        AllSeeingWardenBrain::removeDeadSuspect, false),
                LookAtMobTask.create((LivingEntity entity) -> AllSeeingWardenBrain.isTargeting(warden, entity),
                        (float) warden.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE)),
                RangedApproachTask.create(1.6f),
                new SonicBoomTask(),
                MeleeAttackTask.create(18)), MemoryModuleType.ATTACK_TARGET);
    }

    private static boolean isTargeting(AllSeeingWardenEntity warden, LivingEntity entity2) {
        return warden.getBrain().getOptionalMemory(MemoryModuleType.ATTACK_TARGET).filter(entity -> entity == entity2).isPresent();
    }

    private static void removeDeadSuspect(AllSeeingWardenEntity warden, LivingEntity suspect) {
        if (!warden.isValidTarget(suspect)) {
            warden.removeSuspect(suspect);
        }
        AllSeeingWardenBrain.resetDigCooldown(warden);
    }
