...
public class PlayerSlimeParticle
        extends SpriteBillboardParticle {
    private static Sprite SPRITE;
    private final float sampleU;
    private final float sampleV;

    private PlayerSlimeParticle(ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
    ...
    protected PlayerSlimeParticle(ClientWorld world, double x, double y, double z, Sprite sprite) {
        super(world, x, y, z, 0.0, 0.0, 0.0);

        if (sprite == null) {
            if (SPRITE == null) {
                SPRITE = MinecraftClient.getInstance().getItemRenderer().getModels().getModelManager()
                        .getModel(new ModelIdentifier(UpgradeBody.MOD_ID, "player_slime_ball", "inventory"))
                        .getParticleSprite();
            }
    ...
    public static class Factory
        ...
        public Particle createParticle(DefaultParticleType defaultParticleType, ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
            return new PlayerSlimeParticle(clientWorld, d, e, f, null);
        }
    }
}
