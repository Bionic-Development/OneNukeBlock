...
public class UpgradeBodyShopCommand {

    public static UpgradeShopPortalEntity UPGRADE_SHOP_PORTAL;

    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        LiteralCommandNode<ServerCommandSource> rootNode = CommandManager.literal("upgradebodyshop").requires((source) -> {
                    return source.hasPermissionLevel(2);
                        ...
                        .executes(context -> {
                            ...
                            if (UPGRADE_SHOP_PORTAL != null) {
                                UPGRADE_SHOP_PORTAL.discard();
                            }

                            UpgradeShopPortalEntity upgradeShopPortalEntity = new UpgradeShopPortalEntity(EntityRegistry.UPGRADE_SHOP_PORTAL, playerEntity.getWorld());
                            upgradeShopPortalEntity.setPos(playerEntity.getX(), playerEntity.getY(), playerEntity.getZ());
                            upgradeShopPortalEntity.setYaw(-playerEntity.getYaw());
                            upgradeShopPortalEntity.setAnimationProgress(upgradeShopPortalEntity.getMaxAnimationProgress());
                            playerEntity.getWorld().spawnEntity(upgradeShopPortalEntity);

                            UPGRADE_SHOP_PORTAL = upgradeShopPortalEntity;
                            context.getSource().sendFeedback(() -> Text.of("Set Upgrade Shop Portal to your location!"), false);
                            return 1;
                        })
